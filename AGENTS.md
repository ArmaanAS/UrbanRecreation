...
| `rust/` | Imported Rust implementation and candidate high-performance backend. `engine::BaseRulesGame` remains the effects-disabled 20-round reference; `engine::ClanBonusDiagnostic` preserves its separate 40-round projected gate; `engine::CombatStatDiagnosticV1` adds a distinct fail-closed combat-stat projection with a 29-round gate. Its semantic revision 7 includes only the bounded fixed-stat Confidence/Revenge/Frozn previous-round slice, not general temporal-condition parity. `engine::CatalogCombatStatMatchV1` is the strict catalog-only constructor for future search positions: it applies reviewed runtime card overrides, derives immutable Oculus/effective-clan and day/night context, and rejects a draw unless all eight cards are executable by that projection. The historical engine remains beside them, and the old HTTP advisor is behind `legacy-advisor`. New Rust work reads the root canonical data and captures—`rust/assets/` is historical only. See `docs/rust-migration.md`. |
...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 22 sequential prefix rounds.
   ...
   Attack/Power/Damage abilities, and the earlier bonus/control slice; global/off-card
   hazards are preparation-fatal and other unadmitted current-round combat-stat effects
   reject if selected. Ability and bonus Support use independently validated immutable
   effective-clan character counts; provenance revision 6 records the current compiler
   policy.
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 22-round combat-stat diagnostic adds fixed ordinary abilities,
