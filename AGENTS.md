...
| `rust/` | Imported Rust implementation and candidate high-performance backend. `engine::BaseRulesGame` remains the effects-disabled 20-round reference; `engine::ClanBonusDiagnostic` preserves its separate 40-round projected gate; `engine::CombatStatDiagnosticV1` adds a distinct fail-closed combat-stat projection with a 38-round gate. Semantic revision 9 retains the bounded fixed-stat and previous-round slices, exact Defeat recovery (`bonus 577`, `ability 729/1418`), and adds only Riots' exact `Victory Or Defeat : +1 Pillz` bonus (`1034`), not general temporal-condition or resource-effect parity. `engine::CatalogCombatStatMatchV1` is the strict catalog-only constructor for future search positions: it applies reviewed runtime card overrides, derives immutable Oculus/effective-clan and day/night context, and rejects a draw unless all eight cards are executable by that projection. The historical engine remains beside them, and the old HTTP advisor is behind `legacy-advisor`. New Rust work reads the root canonical data and captures—`rust/assets/` is historical only. See `docs/rust-migration.md`. |
...
1. Capture many real PvP games and make the engine reproduce them (`tests/replay/`).
   As of 2026-09-16: **328 battles captured, 322 replay-ready, 277 replay exactly** (life,
   pillz, power, damage, attack, winner per round), **45 mismatch**, and 6 incomplete/Dojo
   captures ignored. Four open cases have already been investigated (874590, 874712,
   901004, 1093173); the other 41 are fresh regression targets from the expanded corpus and
...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 38 sequential prefix rounds.
   ...
   post-round Defeat recovery identities (bonus `577`; abilities `729`/`1418`) and Riots'
   exact Victory-or-Defeat Pillz bonus (`1034`); global/off-card
   ...
   provenance revision 9 records the current compiler policy. Recovery ratios, source-id
   variants (including `2475`), same-text Riots ability aliases, and controls beyond Stop
   Bonus remain fail-closed.
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 38-round combat-stat diagnostic adds fixed ordinary abilities,
