...
- `rust/src/advisor/` is the current-engine vertical slice: strict manual/demo/replay input, a
  ...
  reply, then the revealed card replaces it with precise SECOND advice. Do not present it as
  live-stream capture, refreshed opening data, worker/protocol support, or complete
  TypeScript policy parity.
