...
| `src/decks/` | Deck builder (read-only so far): `SiteData.ts` parses Collection Pro's collection, format and deck records; `DeckFormat.ts` ports the site's own deck validator (agrees with the server on all 76 of the owner's deck-format pairs); `Report.ts` builds a deck report; `Service.ts` serves it (`deno task decks`, reached by the userscript's Collection Pro panel through the log server's `/decks/`) and serves Deck Lab (`ui/`, http://127.0.0.1:8788): a collection browser and deck drafter with live reports, read-only. Data comes from `scripts/DeckCapture.ts`. `Matchup.ts` scores one deck against another on the exact Rust solver (`deno task matchup`, phase 5): seeded hand-pair sampling with common random numbers, both first movers per pair, score = (value A-first - value B-first)/2, refused draws counted and never scored, solves cached under `cache/matchups/` keyed by engine and data provenance. See `docs/deck-builder-design.md` and `docs/site-api.md`. |
...
deno task decks                  # deck service for the Collection Pro panel; open http://127.0.0.1:8788 for Deck Lab
deno task deck-data [logs...]    # rebuild the deck data files from raw logs
...
deno task opening-prior          # recount the round-one reply prior; --write rewrites both literals (a policy-revision change)
UR_SLOW_PARITY=1 deno test -A --no-check tests/solver/ExactOpeningParity.test.ts tests/solver/PolicyCache.test.ts  # slow gates: round-one TS/Rust parity, round-one cache proof
...
1. Capture many real PvP games and make the engine reproduce them (`tests/replay/`).
   As of 2026-09-26: **383 battles captured, 377 replay-ready, 369 replay exactly** (life,
...
2. **In progress: deck building for a game mode.** The owner's next bigger idea
   ...
   first cut of phase 3 (Deck Lab) and the phase 5 engine (`src/decks/Matchup.ts`,
   `deno task matchup`, `deno task card-coverage`); the rest of that doc - a guarded apply, a
...
2. New games: the owner runs `deno task advise` (or standalone `deno task log` for capture
   ...
   is at 0.9.0 (the log server warns when the browser's copy differs; open
   http://localhost:8787/ur-logger.user.js to update) - if `ur_log.jsonl` starts growing by gigabytes again, Tampermonkey is still
...
2. Card data was complete as of 2026-09-26 (2498 cards, every level, 36 clans). To refresh:
   `__ur.dumpCharacters()` in a `/game/play/webgl/` tab (the script borrows that tab's API
   access) and `__ur.dumpClans()`, with the log server running, then `deno task cards`, then
   `deno task extract`. `deno task cards` refuses a catalog that would drop cards (a dump cut
   short by an expired token used to look complete). A new clan must also be added to
   `Clans` in `src/game/types/CardTypes.ts`. Cards that became collectors got a " Cr" suffix;
   the loader keeps the old name as an alias. **A refresh can rebalance cards the corpus has
   already captured**: the 2026-09-26 one changed abilities of Trasher-X L4, Chelonite L3,
   P. Steevens Cr L2-3, DJ Korr Cr L4 and Quetzal Cr L3 and powers of Horace L2. The
   extractor therefore writes the battle's own text into the testcase's `abilities` slot
   whenever it differs from today's catalog (as it already did for Hazard), so old captures
   keep replaying with what the server dealt. Stat-only changes have no such slot yet; no
   captured game has needed one. Strict Rust eligibility uses today's catalog, so a
   rebalanced card whose new ability id no capture has shown drops out of it.
...
- Round one is solved exactly in both implementations, like rounds 2-4 (the owner's call,
  2026-09-26: the one-round position heuristic "wasn't very good"). The heuristic and every
  flag or wire field that chose it are gone, and the advisor policy semantic revision is 3.
  The only round-one difference left is the weighting: the opponent's current reply is
  weighted by `OPENING_REPLY_COUNTS`, a literal table in `Search.ts` and `search.rs` of 380
  captured opponent round-one plays (never the owner's), regenerated only by `deno task
  opening-prior --write` and held equal in both languages and to its recount by
  `tests/solver/OpeningPrior.test.ts`. `openingPrior` (TS) / `weights_by_opening_prior`
  (Rust) name that axis. Both policies remember solved positions (the continuation caches,
  see "Performance"), which is what made this affordable: a round-one FIRST solve takes
  about 0.9-1.6 s in the Rust worker on six threads, 9-13 s in one TypeScript `Search` and
  5-6 s in `ParallelSearch` with three workers; SECOND is 4-8x cheaper. The V3 request has no
  `opening_policy`, and a response claiming `opening_estimate` is rejected. The host now
  gives the Rust worker 30 s when no budget is set, and the Rust advisor defaults to 10 s.
- `rust/src/advisor/` is the current-engine vertical slice: strict manual/demo/replay input, a
  ...
  advancing. Every round uses an exact conservative policy whose reply may depend on a
  visible opposing card but not hidden pillz/Fury; round 1 weights the opponent's reply by
  the opening prior. In manual rounds 2-4 where the opponent moves first, a
...
- **Shape.** `iterTree` is breadth-first over the whole tree, so no answer exists until it
  ...
  worker pool splits those units across processes in every round, round 1 included,
  which is solved exactly like the others since 2026-09-26. `ParallelSearch` deals whole
  card pairs (ours, theirs) to workers rather than single units, because each worker owns
  its continuation cache and a round's repeats live within a card pair: three workers went
  from 9.98 s to 6.20 s on 877636's opening FIRST (one thread 12.6 s). Round-two searches
  are still too small for the pool to pay (about 205 ms on one thread against 310 ms on
  three).
...
- **What the percentage means.** In every round each root move is averaged over the
  opponent's possible current replies (weighted by the captured opening prior in round 1,
  uniformly afterwards; this is still a model rather than an empirical probability). Every later continuation is evaluated by `Policy.ts` as a
...
`deno task time-search` from 1.69 s to 0.47 s; the continuation cache (2026-09-26) then took
it to 0.20 s. Both of the first changes came from profiling,
...
- **TypeScript: the continuation policy remembers positions too (2026-09-26).** `Policy.ts`
  caches each completed round value in a `ContinuationCache` owned by the `PolicyStack`, so
  one per `Search` or `ParallelSearch` worker, bound to one match through
  `Game.matchTables` and capped at 2^19 entries. The TypeScript `Game` is not one position
  record, so the key is assembled from an audit of `Game.make`/`Undo`: `id`, `winner`, the
  asker and first mover, both packed Players and PlayerRounds, `lastClan`, the played masks,
  and each side's `Events.repeat` entries serialised by structure with their won/delayed
  flags. 877636's exact opening went from 283 s to 12.6 s for FIRST and 17.3 s to 1.64 s for
  SECOND, and `deno task time-search` from 0.47 s to 0.20 s, all with identical checksums.
  `tests/solver/PolicyCache.test.ts` compares cached and uncached values over five hand
  pairs (latch, Backlash, delayed Poison/Heal, Growth freeze with Revenge, After [clan] and
  Oculus, night). **Anything new that a round can change and a later battle reads must go
  into that key** (or into `Events.repeat`, which the key already serialises), or the cache
  will silently return wrong values.
...
- Each card in a battle snapshot carries a `state`, which is a property of that player's
  *copy* of the card and not of the card itself. Seen so far: `""` (plain), `"p"`, `"s"`,
  and `"m1"` / `"m2"` / `"m3"`. The site names all sixteen (Collection Pro's edition filter,
  2026-09-26): `""` Classic, `p` Prismatic, `s` Savage, `ga` Golden Aura, `gs` Golden Savage,
  `m1` Sapphire Deep Blue, `a1` Andromeda, `k1` Knight Blaze, `v1` Void, `c1` Cold, `g1` Glam,
  `ar1` Arcade, `m2` Frost Platinum, `m3` Wonder Spectrum, `rp` Rampage and `i` Immortal
  Legacy (the one edition that can also be devolved). So `m1`-`m3` are three named editions,
  not tiers of one, and like the others they belong to a copy, not to the card.
