...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 87 unique sequential prefix rounds.
   ...
   exact Equalizer opponent-Life definitions `1415`/`4458`, and Anita level 3's exact
   `ability:274` Courage conversion from final resolved damage to Life,
   ...
   provenance revision 19 records the current compiler policy. Recovery ratios, source-id
   variants (including `2475`), every unlisted same-text identity (including Ellie/Lorea's
   Anita-like text), Anita levels 1–2, Bonus/Copy provenance, malformed Anita records, generic capped increases,
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 87-round combat-stat diagnostic adds fixed ordinary abilities,
  ...
  and ordinary numeric Support abilities without claiming general condition or full-effect parity.
...
- `rust/src/advisor/` is the current-engine vertical slice: strict manual/demo/replay input, a
  ...
  supported captures `877636`, `877812`, `925719`, `1024673`, `1060199`, `1061897`,
  `1069813`, `1081463`, and `1089346`.
