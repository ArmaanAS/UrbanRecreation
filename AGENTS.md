...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 289 unique sequential prefix rounds.
   ...
   provenance revision 33 records the current compiler policy. Every other prefixed permanent
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 289-round combat-stat diagnostic adds fixed ordinary abilities,
