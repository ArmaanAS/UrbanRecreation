...
1. Capture many real PvP games and make the engine reproduce them (`tests/replay/`).
   As of 2026-09-24: **383 battles captured, 376 replay-ready, 345 replay exactly** (life,
   pillz, power, damage, attack, winner per round), **31 mismatch**, and 7 captures ignored:
   ...
   1093173); the other 28 are fresh regression targets from the expanded corpus and
   remain mostly untriaged (942983 and 943111 only on a stale last round), and 1414749
   arrived with the 2026-09-23 captures. This is fresh ground truth rather than evidence that earlier working
   ...
   was fixed and what is open (Revenge/Impose, abbreviated condition prefixes, and
   single-point rules waiting for a second capture). It also says which open questions need more
...
- `EffectiveCardCatalog` is required for solver-facing construction and validates
  ...
  revision 68, catalog-context revision 4).
