...
| `rust/` | Imported Rust implementation and candidate high-performance backend. `engine::BaseRulesGame` remains the effects-disabled 20-round reference; `engine::ClanBonusDiagnostic` preserves its separate 40-round projected gate; `engine::CombatStatDiagnosticV1` adds a distinct fail-closed combat-stat projection with a 52-round gate. Semantic revision 13 retains the bounded numeric and post-round slices and adds exact unconditional Stop Opp. Ability with allocation-free TypeScript-compatible PRE4 dependency resolution; conditional SOA, Protection, Copy, capped increases, and unlisted resource effects remain fail-closed. `engine::CatalogCombatStatMatchV1` is the strict catalog-only constructor for search positions: it applies reviewed runtime card overrides, derives immutable Oculus/effective-clan and day/night context, and rejects a draw unless all eight cards are executable by that projection. `advisor/` contains a current-engine make/unmake search, bounded TUI, manual four-round mode, a fixed 198-play historical opening prior, exact conservative rounds 2–4 policy, a single-thread manual blind-second pass, and strict captured-replay grading. The precompiled one-request JSONL worker lets the TypeScript host use supported FIRST decisions after validating provenance, replayed history, the legal action set, and response bounds; SECOND/blind integration remains TypeScript-only. Complete captures `877636` and `1024673` are current end-to-end gates, not full live-policy parity. The historical engine remains beside them, and the old HTTP advisor is behind `legacy-advisor`. New Rust work reads the root canonical data and captures—`rust/assets/` is historical only. See `docs/rust-migration.md`. |
...
deno task advise --rust=compare  # TS stays authoritative; compare supported Rust FIRST decisions
deno task advise --rust=use      # use complete protocol-validated Rust FIRST results, else TS fallback
...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 52 sequential prefix rounds.
   ...
   fixed `+N Life` on victory, plus exact unconditional Stop Opp. Ability from Ability,
   Roots, and GHEIST sources using source-dependency ordering shared with Stop Bonus;
   ...
   provenance revision 13 records the current compiler policy. Recovery ratios, source-id
   variants (including `2475`), every unlisted same-text identity, generic capped increases,
   and controls beyond unconditional Stop Bonus/Stop Opp. Ability remain fail-closed.
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 52-round combat-stat diagnostic adds fixed ordinary abilities,
...
- `rust/src/advisor/` is the current-engine vertical slice: strict manual/demo/replay input, a
  ...
  supported captures `877636` and `1024673`. Replay mode verifies complete card and resource results before
  ...
  can now launch the precompiled `rust:worker` in a strict one-request JSONL V1 boundary for
  supported FIRST decisions: `--rust=compare` retains TS authority, while `--rust=use` accepts
  only structurally validated complete Rust results and otherwise falls back to TS. Default is
  off; provenance, replayed history, the legal action set, and response bounds are strictly
  checked, without claiming per-request semantic equivalence. SECOND/blind
  decisions remain TypeScript-only in the integrated live path. Do not present this as
  refreshed opening data or complete TypeScript policy parity.
