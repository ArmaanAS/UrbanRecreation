...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 124 unique sequential prefix rounds.
   ...
   provenance revision 23 records the current compiler policy. Recovery ratios, source-id
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 124-round combat-stat diagnostic adds fixed ordinary abilities,
  ...
  unconditional and reviewed conditional Victory opponent-Life, unconditional and Reprisal/Revenge Copy, the three reviewed Protection grammars, and ordinary numeric Support abilities without claiming general condition or full-effect parity.
