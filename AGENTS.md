...
| `rust/` | Imported Rust implementation and candidate high-performance backend. `engine::BaseRulesGame` remains the effects-disabled 20-round reference; `engine::ClanBonusDiagnostic` preserves its separate 40-round projected gate; `engine::CombatStatDiagnosticV1` adds a distinct fail-closed combat-stat projection with a 31-round gate. Semantic revision 8 retains the bounded fixed-stat Confidence/Revenge/Frozn previous-round slice and adds only exact Defeat recovery (`bonus 577`, `ability 729/1418`), not general temporal-condition or resource-effect parity. `engine::CatalogCombatStatMatchV1` is the strict catalog-only constructor for future search positions: it applies reviewed runtime card overrides, derives immutable Oculus/effective-clan and day/night context, and rejects a draw unless all eight cards are executable by that projection. The historical engine remains beside them, and the old HTTP advisor is behind `legacy-advisor`. New Rust work reads the root canonical data and captures—`rust/assets/` is historical only. See `docs/rust-migration.md`. |
...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 31 sequential prefix rounds.
   ...
   post-round Defeat recovery identities (bonus `577`; abilities `729`/`1418`); global/off-card
   ...
   provenance revision 8 records the current compiler policy. Recovery ratios, source-id
   variants (including `2475`), and controls beyond Stop Bonus remain fail-closed.
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 31-round combat-stat diagnostic adds fixed ordinary abilities,
