...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 59 sequential prefix rounds.
   ...
   provenance revision 16 records the current compiler policy. Recovery ratios, source-id
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 59-round combat-stat diagnostic adds fixed ordinary abilities,
