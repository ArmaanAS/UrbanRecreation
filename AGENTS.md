...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 189 unique sequential prefix rounds.
   ...
   and Lianah Ld's exact `Heal 1 Max. 20` (`3526`), the first latched permanent: a live source
   whose card wins writes the Heal into its owner's `BaseRulesPosition::latched`, that round
   pays nothing, and every later round then pays one Life after the round's own effects
   while the owner is living and below 20 (the KO stays terminal; undo restores the latch);
   ...
   provenance revision 26 records the current compiler policy. Every other permanent -
   Poison, Toxin, Regen, the other `Heal N Max. M` identities and the `Defeat`/`Asymmetry`
   Heal prefixes - stays visible-but-disabled; Lianah's id or exact text under any other
   shape is a selected hazard. Recovery ratios, source-id
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 189-round combat-stat diagnostic adds fixed ordinary abilities,
