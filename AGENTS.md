...
| `rust/` | Imported Rust implementation and candidate high-performance backend. `engine::BaseRulesGame` remains the effects-disabled 20-round reference; `engine::ClanBonusDiagnostic` preserves its separate 40-round projected gate; `engine::CombatStatDiagnosticV1` adds a distinct fail-closed fixed combat-stat projection with a 13-round gate. The historical engine remains beside them, and the old HTTP advisor is behind `legacy-advisor`. New Rust work reads the root canonical data and captures—`rust/assets/` is historical only. See `docs/rust-migration.md`. |
...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 13 sequential prefix rounds.
   It admits only fixed ordinary combat stats with Always/Courage/Reprisal and numeric
   Symmetry/Asymmetry (immutable original hand-slot equality/inequality), plus the earlier
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 13-round combat-stat diagnostic adds fixed ordinary abilities and
  numeric hand-slot predicates without claiming general condition or full-effect parity.
