...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 200 unique sequential prefix rounds.
   ...
   and the plain `Heal N Max. M` ability grammar (eight structurally identical records, from
   Lianah Ld's `3526` to Campbell's `963`), the first latched permanent: a live source whose
   card wins writes the Heal into its owner's `BaseRulesPosition::latched`, that round pays
   nothing, and every later round then pays N Life after the round's own effects while the
   owner is living and below M (the KO stays terminal; undo restores the latch);
   ...
   provenance revision 27 records the current compiler policy. Every other permanent -
   Poison, Toxin, Regen and the `Defeat`/`Asymmetry`/clan-gated Heal prefixes - stays
   visible-but-disabled; plain `Heal` text in a Bonus slot or over a malformed shape, and the
   complete permanent shape under disagreeing numbers, are selected hazards. Recovery ratios, source-id
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 200-round combat-stat diagnostic adds fixed ordinary abilities,
