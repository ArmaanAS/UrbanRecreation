...
4. Prefer fixes backed by ≥2 captured data points; note single-point hypotheses in the
   ...
   The separate Rust `CombatStatDiagnosticV1` gate is fixed at 273 unique sequential prefix rounds.
   ...
   its Symmetry form (`1852`) paid to the winner as its final resolved Damage, exact structured
   ...
   provenance revision 31 records the current compiler policy. Every prefixed permanent
...
- `rust/src/engine/` keeps base rules and projected effect models separate. The 20-round
  ...
  bonus slice, and the 273-round combat-stat diagnostic adds fixed ordinary abilities,
  ...
  unconditional and reviewed conditional Victory opponent-Life, unconditional, Reprisal/Revenge and Asymmetry source Copy, unconditional stat Copy, the three reviewed Protection grammars, Attack per opposing Damage, Defeat opponent-Life, the four latched plain permanent Life grammars, plain own and opposing Victory Pillz, Pillz per Damage, and ordinary numeric Support abilities without claiming general condition or full-effect parity.
