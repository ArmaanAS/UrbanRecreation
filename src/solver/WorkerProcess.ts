...
import { FlowController } from "../utils/Utils.ts";
import { Node } from "./Minimax.ts";
...
export default class WorkerProcess<InputType extends object, OutputType> {
  ...
  startProcess() {
    ...
    this.worker.once("message", (_data) => {
  ...
  static async processOnWorker<I extends object, O extends object>(
    data: I,
  ): Promise<O> {
    if (!this.init) {
      this.init();
