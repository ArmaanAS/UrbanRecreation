type PromiseCallback = (item: unknown) => Promise<unknown>;
...
export default class CallbackPromiseMap {
  ...
  max: number;
  id: number;
  pending: Map<number, Promise<unknown>>;
  constructor(items: unknown[], cb: PromiseCallback, max = 3) {
  ...
  static async race(items: unknown[], cb: PromiseCallback, max: number) {
