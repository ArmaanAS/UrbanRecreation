...
fn strict_catalog_coverage_of_all_complete_captured_draws_is_pinned() {
    ...
    support::expect_count(
        "catalog_coverage_scanned_draws",
        "Captured games with two complete four-card hands that the scan considered.",
        scanned,
    );
    support::expect_ids(
        "catalog_coverage_eligible_draws",
        "Captured draws the strict catalog-only projection accepts in full.",
        &eligible,
    );
}
