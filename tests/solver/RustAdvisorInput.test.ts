...
Deno.test("Rust V1 normalises 1024673's opening into engine P1 and preserves null identities", async () => {
...
Deno.test("Rust V1 maps 1024673's round two into P2-first history and server resources", async () => {
...
Deno.test("Rust V1 provenance is the Rust FNV-1a contract over raw repository bytes", async () => {
  const expected = {
...
Deno.test("Rust V1 rejects non-first decisions before it starts a worker", async () => {
  const rec = openingCapture();
  const result = await normaliseRustAdvisorInput({
    rec,
    game: freshGame(rec),
  ...
  assertUnsupported(result, "mode is second, not first");
});

Deno.test("Rust V1 accepts a live-shaped empty current round", async () => {
...
Deno.test("Rust V1 rejects a partial current round and chronology tails", async () => {
...
Deno.test("Rust V1 rejects a first-mode decision when the capture owner is not first", async () => {
  const rec = openingCapture();
  rec.mySide = 0;
  const result = await normaliseRustAdvisorInput({
    rec,
...
Deno.test("Rust V1 forwards authoritative server resources despite TS replay drift", async () => {
